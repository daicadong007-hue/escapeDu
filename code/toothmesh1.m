...
for i=1:1001
    ...
    d=r_b*((beta+teta_b)*sin(beta)+cos(beta));
    u_f=d-r_d-h*tan(beta);
    ...
    for j=1:1000
        ...
        x1=r_b*((beta1+teta_b)*sin(beta1)+cos(beta1));
        beta2=(j)*dbetaj+beta_0;
        h2=r_b*((beta2+teta_b)*cos(beta2)-sin(beta2));
        x2=r_b*((beta2+teta_b)*sin(beta2)+cos(beta2));
        dx=x2-x1;
        if h2>h_q
            h2=h_q;
        end
    ...
    dx=(r_b*cos(teta_b)-r_d*cos(teta_f))/1000;
    for j=1:1000
        x1=r_d*cos(teta_f)+(j-1)*dx;
        h1=h0;
        x2=r_d*cos(teta_f)+(j)*dx;
        h2=h0;
        dx=x2-x1;
        if h2>h_q
            h2=h_q;
